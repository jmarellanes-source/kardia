...
GO
CREATE PROCEDURE [PO].[SP_COM_CASTIGOS_REVERSO_MORATORIOS]
(
	@FECHA_INICIO DATE
)
AS
BEGIN
	...
	BEGIN TRY
		BEGIN TRANSACTION
			...
			WHERE C.IND_ESTADO IN ('C','D')
					...
					AND B.MON_PAGO <> 0.00
					AND A.ID_CUENTA IN ('1401030090010000','1401030090020000')
					AND B.COD_CONCEPTO IN ('MORATORIOS', 'INTERES_MORATORIO', 'MORA_EMPRESARIAL')

		-- Del 20 a fin de mes puede haber reverso de moratorios por llegada de pago antes de corte
		IF DAY(@FECHA_INICIO) >= 20 BEGIN
			INSERT INTO PO.SAF_MOV_ABONO(
				ID_EXTERNO, NUM_CREDITO, NOM_CLIENTE, CUENTA_CARGO, CUENTA_ABONO, ID_PRODUCTO, NOMBRE_PRODUCTO, ID_FONDEO, RUBRO, MONTO
				,MON_PAGO_IVA, IND_FORMA_PAGO, ID_TIPO_CREDITO, TIPO_CREDITO, IND_ESTADO, CARTERA_VENCIDA, FECHA_MOVIMIENTO
				,TIP_TRANSACCION, SUBTIP_TRANSAC, IND_ETAPA, MON_CREDITO, CEDIDO, FEC_APERTURA, ID_SECUENCIA, ID_REFERENCIA, ID_CUENTA, IND_COVID, 
				CONCEPTO_FINAL, TABLA_ORIGEN, ID_POLIZA, TIPO_VIVIENDA, CANAL)
			SELECT C.ID_EXTERNO
				,C.NUM_CREDITO
				,dbo.FN_FORMA_NOMBRE(PF.PRIMER_APELLIDO, PF.SEGUNDO_APELLIDO, PF.PRIMER_NOMBRE, PF.SEGUNDO_NOMBRE, PJ.RAZON_SOCIAL, PJ.NOM_COMERCIAL)
				,'' AS CUENTA_CARGO
				...
				,CP.NOMBRE_CORTO
				,CH.ID_FONDEO
				,Q.COD_RUBRO AS RUBRO
				,-1 * Q.MON_RUBRO AS MONTO
				,-1 * Q.MON_IVA AS MON_PAGO_IVA
				,0 AS IND_FORMA_PAGO
				,C.NUM_DESC_TIPO_CREDITO
				,CP.NOMBRE_CORTO
				,C.IND_ESTADO
				...
				,''
				,dbo.FN_TIPO_CANAL(C.TIP_TASA)
			FROM Quiero_Confianza_shadow.PR.PR_RUBRO_COBRO_X_CREDITO Q WITH (NOLOCK)
				INNER JOIN Quiero_Confianza_shadow.PR.PR_CREDITOS C WITH (NOLOCK) ON Q.COD_EMPRESA = C.COD_EMPRESA AND Q.NUM_CREDITO = C.NUM_CREDITO
				INNER JOIN Quiero_Confianza_shadow.PR.PR_TIPO_CREDITO P WITH (NOLOCK) ON C.TIP_CREDITO = P.TIP_CREDITO AND C.COD_EMPRESA = P.COD_EMPRESA
				LEFT JOIN Quiero_Confianza_shadow.CL.CL_PERSONAS_JURIDICAS PJ WITH (NOLOCK) ON C.COD_EMPRESA = PJ.COD_EMPRESA AND C.COD_CLIENTE = PJ.COD_CLIENTE
				LEFT JOIN Quiero_Confianza_shadow.CL.CL_PERSONAS_FISICAS PF WITH (NOLOCK) ON C.COD_EMPRESA = PF.COD_EMPRESA AND C.COD_CLIENTE = PF.COD_CLIENTE
				LEFT JOIN PO.PR_CREDITO_EXTRA CE ON CE.ID_EXTERNO = C.ID_EXTERNO
				LEFT JOIN PO.PR_CREDITO_PRODUCTO CP ON CP.ID_PRODUCTO = CE.ID_PRODUCTO
				LEFT JOIN PO.CONTROL_HISTORICO_TRASPASOS CH ON Q.NUM_CREDITO = CH.NUM_CREDITO AND CH.FECHA_SISTEMA = @FECHA_INICIO
			WHERE C.IND_LINEA = 'N'
				AND (C.FEC_CANCELACION is null or @FECHA_INICIO <= C.FEC_CANCELACION)
                    AND C.NUM_DESC_TIPO_CREDITO IN (25,   29)
                    AND C.COD_ORIGEN NOT IN (22,23,28, 35)
				AND Q.IND_ESTADO = 'N'
				AND Q.COD_RUBRO IN ('MORATORIOS')
				AND Q.FEC_COBRO = @FECHA_INICIO
		END

		-- Reversos por movimientos en tickes
		INSERT INTO PO.SAF_MOV_ABONO(
		    ID_EXTERNO, NUM_CREDITO, NOM_CLIENTE, ID_PRODUCTO, NOMBRE_PRODUCTO, ID_FONDEO, RUBRO, MONTO, MON_PAGO_IVA, ID_TIPO_CREDITO, 
		    TIPO_CREDITO, FECHA_MOVIMIENTO, IND_ESTADO, IND_ETAPA, CEDIDO, FEC_APERTURA, ID_POLIZA, TIPO_VIVIENDA,
		    TABLA_ORIGEN, CANAL)
		SELECT  
			C.ID_EXTERNO
			,A.NUM_CREDITO
		...
		FROM PO.SAF_RUBRO_HIST A WITH (NOLOCK)
			...
			LEFT JOIN Quiero_Confianza_shadow.CL.CL_PERSONAS_JURIDICAS PJ WITH (NOLOCK) ON C.COD_EMPRESA = PJ.COD_EMPRESA AND C.COD_CLIENTE = PJ.COD_CLIENTE
			LEFT JOIN Quiero_Confianza_shadow.CL.CL_PERSONAS_FISICAS PF WITH (NOLOCK) ON C.COD_EMPRESA = PF.COD_EMPRESA AND C.COD_CLIENTE = PF.COD_CLIENTE
		WHERE A.COD_RUBRO IN ('MORATORIOS', 'INTERES_MORATORIO', 'MORA_EMPRESARIAL')
		...
		UPDATE PO.SAF_MOV_ABONO
		SET CONCEPTO_ANT = 'CMR CAS DEV REVERSO'
        WHERE FECHA_MOVIMIENTO = @FECHA_INICIO
			AND ID_POLIZA = @ID_POLIZA
 ...
 ---------------------------------------- FIN REGISTRO DE CASTIGO EN CUENTAS DE ORDEN ----------------------------
		...
		UPDATE PO.SAF_MOV_ABONO
		SET CONCEPTO_FINAL= LEFT(CONCAT('CMR CAS DEV REV '
