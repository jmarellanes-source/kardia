...
GO
CREATE PROCEDURE [PO].[SP_COM_COBRANZA_CASTIGOS]
(
	@FECHA_INICIO DATE
)
AS
BEGIN
	...
	BEGIN TRY
		BEGIN TRANSACTION
			...
			WHERE C.IND_ESTADO IN ('C', 'D')
					...
					AND (B.MON_PAGO <> 0.00 OR B.MON_PAGO_IVA <> 0)
			--# END_BLOCK: EXTRACTOR
		...
		UPDATE PO.SAF_MOV_ABONO
		SET CONCEPTO_ANT = CONCAT('CMR PAGO'
                ,CASE
 ...
 ---------------------------------------- FIN REGISTRO DE CASTIGO EN CUENTAS DE ORDEN ----------------------------
	    ...
	    --UPDATE PO.SAF_MOV_ABONO SET TIPO_DEP = 3 WHERE FECHA_MOVIMIENTO = @FECHA_INICIO AND ID_POLIZA = @ID_POLIZA AND ID_TIPO_CREDITO = 4
	    --UPDATE PO.SAF_MOV_ABONO SET TIPO_DEP = 4 WHERE FECHA_MOVIMIENTO = @FECHA_INICIO AND ID_POLIZA = @ID_POLIZA AND ID_TIPO_CREDITO = 4 AND RUBRO = ''


		UPDATE PO.SAF_MOV_ABONO
			SET CONCEPTO_FINAL= LEFT(CONCAT('CMR ', 
				CASE
		...
		FROM #POLIZA
		--WHERE CONCEPTO LIKE 'DEP %' OR CONCEPTO LIKE 'CTA AFI%' OR CUENTA LIKE '7017-%'
