...
GO
CREATE PROCEDURE [PO].[SP_COM_CASTIGOS_REVERSO_COMISIONES]
(
	@FECHA_INICIO DATE
)
AS
	...
	BEGIN TRY
		BEGIN TRANSACTION
			...
			WHERE C.IND_ESTADO IN ('C','D')
					...
					AND B.COD_CONCEPTO IN ('COMISION_ADMINISTRACION', 'COMISION_PREPAGO',
					'ASE_FINANCIERA','COM_PRORROGA','COM_REESTRUCTURA', 'OPI_VALOR','VISITA_EXTRA',
					'CARGO_PORCENTAJE','CARGO_SEGURO','SEG_DAN','SEGURO_DANOS','SEGURO_VIDA','GASTOS_NOTARIALES')

		-- Eliminacion por ticket de soporte
		INSERT INTO PO.SAF_MOV_ABONO(
            ...
            , IND_COVID,TABLA_ORIGEN, ID_POLIZA, TIPO_VIVIENDA, CANAL)
		SELECT C.ID_EXTERNO
			,A.NUM_CREDITO
			,dbo.FN_FORMA_NOMBRE(PF.PRIMER_APELLIDO, PF.SEGUNDO_APELLIDO, PF.PRIMER_NOMBRE, PF.SEGUNDO_NOMBRE, PJ.RAZON_SOCIAL, PJ.NOM_COMERCIAL)
			,ISNULL((SELECT TOP 1 NUM_PRODUCTO FROM PO.PR_CAT_PRODUCTO WHERE ID_SAF = P.TIP_CREDITO), P.TIP_CREDITO) ID_PRODUCTO
		...
		WHERE A.COD_RUBRO IN ('COMISION_ADMINISTRACION', 'COMISION_PREPAGO',
			    'ASE_FINANCIERA','COM_PRORROGA','COM_REESTRUCTURA', 'OPI_VALOR','VISITA_EXTRA',
				'CARGO_PORCENTAJE','CARGO_SEGURO','SEG_DAN','SEGURO_DANOS','SEGURO_VIDA','GASTOS_NOTARIALES')
